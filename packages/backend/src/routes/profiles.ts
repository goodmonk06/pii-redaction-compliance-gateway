...
export async function profileRoutes(fastify: FastifyInstance) {
  ...
  fastify.get<{ Params: { id: string } }>('/profiles/:id', async (request, reply) => {
    ...
    if (!profile) {
      throw new NotFoundError('Profile', request.params.id);
    }
  ...
  fastify.post<{ Params: { id: string } }>(
    ...
    async (request, reply) => {
      ...
      const run = await prisma.redactionRun.create({
        data: {
          profileId,
          inputSampleJson: sanitizeSample(body.data),
          outputSampleJson: sanitizeSample(result.redactedData),
...
// Helper function to sanitize samples (limit size)
function sanitizeSample(data: any): any {
